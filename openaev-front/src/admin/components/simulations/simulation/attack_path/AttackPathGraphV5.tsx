...
interface Props {
  ...
  /** Called when user clicks a path line; receives the ASSET node_ids of that path */
...
const AttackPathGraphV5: FunctionComponent<Props> = ({
  nodes,
  edges,
  paths,
    ...
    <div style={{ position: 'relative', width: '100%', height, overflow: 'hidden' }}>
      ...
      >
        ...
        {paths.map((path) => {
          ...
          // successIds = nodes before and including the split point → solid red
          // failedIds  = nodes from split point onward → dashed green/orange (attack stopped)
          ...
          let sw: number;
          if (!hasSelection) { baseOpacity = 0.75; sw = 2.5; }
          else if (isActive)  { baseOpacity = 1.00; sw = 3.5; }
          else                { baseOpacity = 0.06; sw = 2.0; }
          ...
          return (
            <g key={path.path_id}>
              {/* Wide transparent hit area for clicking the path */}
              {(() => {
                const hitD = !isFailed ? buildPathD(path.node_ids) : (successD || failedD || buildPathD(path.node_ids));
                if (!hitD) return null;
                return (
                  <path
                    d={hitD}
              ...
              {/* Render segments (pointer-events: none so hit area takes precedence) */}
              <g style={{ pointerEvents: 'none' }}>
                {/* Successful segment — red (attacker reached these nodes) */}
                {successD && (
                ...
                {/* Failed segment — dashed green or orange (attack stopped here) */}
                {failedD && (
                ...
                {/* Dead-end marker — green (prevented) or orange (detected) */}
        ...
        {/* Path legend — clickable to highlight path — rendered as HTML overlay so it stays fixed-size during zoom */}
      </svg>
          ...
          {paths.map((path) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span style={{ fontSize: 9, opacity: 0.5, marginLeft: 2, flexShrink: 0 }}>✕</span>
                )}
              </div>
            );
          })}
...
export default AttackPathGraphV5;
