...
const INJECTOR_SPACING = 210; // base vertical distance between injector nodes
const INJECTOR_R      = 38;   // injector hexagon "radius"
...
const AttackPathGraphV4U: FunctionComponent<Props> = ({
  ...
  // Auto-expand injector tool groups when an endpoint is selected
  useEffect(() => {
    ...
    setAutoExpandedInjectors((prev) => {
      const relevant = new Set(
        injectorGroups
          .filter((g) => g.contracts.some((c) => c.targetAssetId === selectedActionNodeId))
          .map((g) => g.tool),
      );
      if (relevant.size === 0) return prev;
      const next = new Set(prev);
      for (const t of relevant) next.add(t);
      return next;
    });
  ...
  const FG_COL_W     = 148;   // horizontal pitch between node centres
  const FG_ROW_H     = 118;   // vertical pitch between node centres
  const FG_MAX_COLS  = 4;
  ...
  const FG_STACK_GAP = 36;    // vertical gap between stacked groups
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height, overflow: 'hidden' }}>
      ...
      >
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            RECON ARROWS — only visible when an endpoint node is clicked
            • No endpoint selected → no arrows
            • Injector collapsed → aggregate arrow from injector hex to endpoint
            • Injector expanded  → per-contract arrows from contract nodes
        ═══════════════════════════════════════════════════════════════════ */}
        {injectorGroups.map((group) => {
          ...
          if (!isExpanded) {
            // Collapsed: single aggregate arrow from injector hex → targeted endpoint
            const injPos = injectorPositions.get(group.tool);
            if (!injPos) return null;
            const hasTarget = group.contracts.some((c) => c.targetAssetId === selectedActionNodeId);
            if (!hasTarget) return null;
            const tp = coordMap.get(selectedActionNodeId);
            if (!tp) return null;
            const dist = Math.sqrt((tp.cx - injPos.cx) ** 2 + (tp.cy - injPos.cy) ** 2);
            if (dist < INJECTOR_R + NODE_R + 20) return null;
            ...
            const endX   = tp.cx  - ux * (NODE_R + 3);        const endY   = tp.cy  - uy * (NODE_R + 3);
            const pts = arrowPoints(injPos.cx, injPos.cy, tp.cx, tp.cy, INJECTOR_R, NODE_R);
            return (
              <g key={`recon-inj-${group.tool}`} style={{ pointerEvents: 'none' }}>
          ...
          const showAll = dimmedGroupExpanded;

          return group.contracts
            .filter(({ targetAssetId }) => showAll || targetAssetId === selectedActionNodeId)
        ...
        {injectorGroups.map((group) => {
          ...
          const selectedAssetNode = selectedActionNodeId
            ...
            : null;
          const dimmedGroupExpanded = expandedDimmedGroups.has(group.tool);
          // Show all contracts if no endpoint selected OR if user expanded the dimmed group
...
export default AttackPathGraphV4U;
