/**
 * AttackPathGraphV1 — Non-Intersecting Finding Groups
 *
 * Based on Variant 4.3 but each finding category is its own exclusive section;
 * endpoints with multiple findings appear in ALL applicable groups.
 *
...
const AttackPathGraphV1: FunctionComponent<Props> = ({
  ...
  useEffect(() => {
    ...
    for (const [nodeId, cats] of assetCategories) {
      ...
      // V1 has no intersection sections — every node belongs to one or more exclusive sections
      for (const cat of cats) idsToExpand.add(cat as string);
    }
  ...
  // ── Finding group layout: each category is its own exclusive section —
  //    endpoints with multiple findings appear in ALL applicable groups ──
  ...
  const { sections: findingSections, zoneBoxes: findingZoneBoxes } = useMemo(() => {
    ...
    const buildSection = (
      ...
      if (expanded) {
        ...
        currentY += height + (isInt ? 0 : FG_STACK_GAP);
    ...
    for (const cat of CATEGORY_ORDER) {
      const allInCat = Array.from(assetCategories.entries())
        .filter(([, cats]) => cats.includes(cat))
        .map(([id]) => id);
      if (allInCat.length === 0) continue;
      const sec = buildSection(cat, 'exclusive', [cat], allInCat);
      sections.push(sec);
      const endY = currentY;
      zoneBoxes.push({
        cat,
        x: FG_START_X - 8,
        y: sec.y - 8,
        width: (sec.width > 0 ? sec.width : FG_CARD_W) + 16,
        height: (sec.y === endY ? FG_CARD_H : (endY - sec.y)) + FG_STACK_GAP + 16,
      });
    }

    return { sections, zoneBoxes };
            ...
            return (
              ...
              return (
                <>
                  ...
                  {findingZoneBoxes.map(({ cat, x, y, width, height }) => {
                    const meta = FINDING_META[cat];
                    return (
                      <g key={`zonebg-${cat}`} style={{ pointerEvents: 'none' }}>
                        <rect x={x} y={y} width={width} height={height} rx={18}
                          fill={`${meta.color}06`} stroke={meta.color}
                          strokeWidth={1.5} strokeOpacity={0.35} strokeDasharray="10 6" />
                      </g>
                    );
                  })}
                  ...
                  {findingSections.filter((s) => s.isExpanded && s.assetIds.length > 0).map((sec) => {
                    const { x, y, width, height, cats, assetIds } = sec;
                    const meta = FINDING_META[cats[0]];
                    const intersectionCount = assetIds.filter((id) => {
                      const c = assetCategories.get(id); return c && c.length > 1;
                    }).length;
                    return (
                      <g key={`fgzone-${sec.id}`}>
                        ...
                        <text x={x + width - 56} y={y + 30} fontSize={10} fill="rgba(255,255,255,0.45)"
                          style={{ pointerEvents: 'none', userSelect: 'none' }}>
                          {assetIds.length} found
                        </text>
                        {intersectionCount > 0 && (
                          <text x={x + 16} y={y + FG_HEADER_H - 6} fontSize={9} fill="rgba(255,255,255,0.38)"
                            style={{ pointerEvents: 'none', userSelect: 'none' }}>
                            +{intersectionCount} also in other groups
                          </text>
                        )}
...
export default AttackPathGraphV1;
