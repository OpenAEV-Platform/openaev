/**
 * AttackPathGraphV4U2 — Attacker Origin Map  (Variant 4 – Updated)
 *
 * Same as V4U but recon arrows (contract → endpoint) are hidden by default.
 * They appear only when:
 *   1. A specific endpoint node is clicked (shows arrows TO that endpoint only), OR
 *   2. An attack path is selected from the top stats bar (shows arrows for
 *      contracts that target nodes in that path).
 */
...
const AttackPathGraphV4U2: FunctionComponent<Props> = ({
  ...
  // V4U2: show recon arrows only when a specific endpoint (ASSET node) is clicked
  //   - If a specific endpoint is clicked → show only arrows to that endpoint
  //   - Otherwise → hide all recon arrows
  const reconVisibleAssetIds = useMemo<Set<string> | null>(() => {
    // Endpoint clicked → show only that endpoint
    if (selectedActionNodeId) {
      const isAsset = nodes.find((n) => n.node_id === selectedActionNodeId && n.node_type === 'ASSET');
      if (isAsset) return new Set([selectedActionNodeId]);
    }
    // Path selected → show all asset nodes in that path
    if (selectedPathId) {
      const pathObj = paths.find((p) => p.path_id === selectedPathId);
      if (pathObj) {
        const assetNodeIds = pathObj.node_ids.filter(
          (id) => nodes.find((n) => n.node_id === id && n.node_type === 'ASSET')
        );
        if (assetNodeIds.length > 0) return new Set(assetNodeIds);
      }
    }
    return null; // null = hide all
  }, [selectedActionNodeId, selectedPathId, nodes, paths]);
  ...
  const FG_PAD       = 28;
  const FG_HEADER_H  = 46;
  const FG_COL_W     = 148;
  const FG_ROW_H     = 118;
  const FG_MAX_COLS  = 4;
  const FG_START_X   = 520;
  const FG_CARD_W    = 220;
  const FG_CARD_H    = 52;
  const FG_STACK_GAP = 36;
  const FG_START_Y   = 60;
  ...
  // Dynamic coord map:
  //   - Path mode: use raw individual positions
  //   - All Paths + group expanded: use stacked grid node positions
  //   - All Paths + group collapsed: use card centre
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height, overflow: 'hidden' }}>
      ...
      >
          ...
          return (
            <g key={path.path_id}>
              ...
              {(isActive || !hasSelection) && path.node_ids.map((nodeId, i) => {
                ...
                const totalOffset = 14 + staggerDir * (bh + 5);
                const bx = pos.mx + pos.px * totalOffset; const by = pos.my + pos.py * totalOffset;
                return (
                  <g key={`${path.path_id}-badge-${i}`} transform={`translate(${bx}, ${by})`} opacity={segOpacity}
                  ...
                  >
                    {totalOffset > 2 && <line x1={0} y1={0} x2={-pos.px * totalOffset} y2={-pos.py * totalOffset} stroke={segColor} strokeWidth={0.8} opacity={0.35} strokeDasharray="2 2" style={{ pointerEvents: 'none' }} />}
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            RECON ARROWS — V4U2: visible when endpoint clicked OR path selected
            • Injector collapsed → aggregate arrow from injector hex
            • Injector expanded  → per-contract arrows
        ═══════════════════════════════════════════════════════════════════ */}
        {reconVisibleAssetIds !== null && injectorGroups.map((group) => {
          const isExpanded = expandedInjectors.has(group.tool) || autoExpandedInjectors.has(group.tool);

          if (!isExpanded) {
            // Collapsed: aggregate arrow from injector hex to each visible target
            const injPos = injectorPositions.get(group.tool);
            if (!injPos) return null;
            const targets = group.contracts
              .filter(({ targetAssetId }) => reconVisibleAssetIds.has(targetAssetId))
          ...
          // Expanded: contract-level arrows
          return group.contracts
            .filter(({ targetAssetId }) => reconVisibleAssetIds.has(targetAssetId))
            .map(({ action, targetAssetId }) => {
            ...
            return (
              <g key={`recon-c-${action.node_id}`} style={{ pointerEvents: 'none' }}>
                ...
                <rect x={ca.labelX - labelLen / 2} y={ca.labelY - 9} width={labelLen} height={17} rx={4}
                  fill="rgba(10,12,22,0.88)" stroke={sc} strokeWidth={0.8} opacity={0.92} />
                <text x={ca.labelX} y={ca.labelY + 4} textAnchor="middle" fontSize={9} fontWeight={600}
        ...
        {injectorGroups.map((group) => {
          ...
          const showAll = !selAsset || dimmedGroupExp;
          if (showAll) {
          ...
          const relevantContracts = group.contracts.filter((c) => c.targetAssetId === selectedActionNodeId);
          const dimmedContracts   = group.contracts.filter((c) => c.targetAssetId !== selectedActionNodeId);
        ...
        {injectorGroups.map((group) => {
          ...
          const showAll = !selectedAssetNode || dimmedGroupExpanded;
...
export default AttackPathGraphV4U2;
