...
type LayoutSection = {
  id: string;
  type: 'exclusive' | 'intersection' | 'multi';
...
function getAssetFindingCategories(
  assetId: string,
  assetNode: AttackPathNode | undefined,
  ...
  // Check the asset node's own credentials
  if (assetNode?.node_credentials_found && assetNode.node_credentials_found.length > 0) cats.add('credentials');
...
const AttackPathGraphV4U5: FunctionComponent<Props> = ({
  ...
  // V4.4: each asset belongs to EXACTLY ONE group based on its finding combination
  const assetCategories = useMemo(() => {
    const map = new Map<string, FindingCategory[]>();
    for (const asset of visibleAssetNodes) {
      map.set(asset.node_id, getAssetFindingCategories(asset.node_id, asset, injectorGroups, nodesInAnyPath));
    }
    return map;
  ...
  useEffect(() => {
    ...
    for (const [nodeId, cats] of assetCategories) {
      ...
      if (cats.length === 1) {
        ...
        idsToExpand.add(`${ordered[0]}|${ordered[1]}`);
      } else if (cats.length >= 3) {
        // Multi-category section — key is exact combination sorted by CATEGORY_ORDER
        const ordered = CATEGORY_ORDER.filter((c) => cats.includes(c));
        idsToExpand.add(ordered.join('|'));
      }
    }
  ...
  // Auto-expand injector tool groups ONLY on endpoint click (not on path select)
  useEffect(() => {
  ...
  }, [selectedActionNodeId, nodes, injectorGroups]);
  ...
  const { sections: findingSections } = useMemo(() => {
    ...
    const intersectionIds = (cat1: FindingCategory, cat2: FindingCategory): string[] =>
      filterIds(
        Array.from(assetCategories.entries())
          .filter(([, cats]) => cats.length === 2 && cats.includes(cat1) && cats.includes(cat2))
          .map(([id]) => id),
      );
    ...
    const buildSection = (
      ...
      const headerH = type === 'exclusive' ? FG_HEADER_H : Math.max(36, cats.length * 16 + 8);
    ...
    for (let i = 0; i < CATEGORY_ORDER.length; i++) {
      ...
      if (excl.length > 0) sections.push(buildSection(cat, 'exclusive', [cat], excl));
      ...
      for (let j = i + 1; j < CATEGORY_ORDER.length; j++) {
        ...
        if (intIds.length > 0) {
          ...
          sections.push(buildSection(key, 'intersection', [cat, cat2], intIds));
        }
      }
    }

    // Per-combination groups for endpoints with 3+ finding categories
    // Each unique combination gets its own group named after its specific findings
    const multiCombMap = new Map<string, string[]>();
    for (const [id, cats] of assetCategories) {
      if (cats.length < 3) continue;
      const key = CATEGORY_ORDER.filter((c) => cats.includes(c)).join('|');
      if (!multiCombMap.has(key)) multiCombMap.set(key, []);
      multiCombMap.get(key)!.push(id);
    }
    for (const [key, ids] of multiCombMap) {
      const filteredIds = filterIds(ids);
      if (filteredIds.length === 0) continue;
      const cats = key.split('|') as FindingCategory[];
      sections.push(buildSection(key, 'multi', cats, filteredIds));
    }
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height, overflow: 'hidden' }}>
      ...
      >
        ...
        {/* ═══════════════════════════════════════════════════════════════════
            RECON ARROWS — V4U5: visible ONLY when an endpoint (ASSET) is clicked
            Shows all actions that ran on that specific endpoint, grouped by injector.
            Injector group can be expanded/collapsed (toggle ✕ button or click group bg).
        ═══════════════════════════════════════════════════════════════════ */}
        {injectorGroups.map((group) => {
          // Only show when the selected node is an ASSET (endpoint), not an action or path
          const clickedEndpoint = selectedActionNodeId
            ? nodes.find((n) => n.node_id === selectedActionNodeId && n.node_type === 'ASSET')
            : null;
          if (!clickedEndpoint) return null;
          // Show only arrows targeting the selected endpoint
          const relevantNodeIds = new Set<string>([clickedEndpoint.node_id]);
                ...
                <>
                  ...
                  {findingSections.filter((s) => s.isExpanded && s.assetIds.length > 0).map((sec) => {
                    ...
                    if (type === 'exclusive') {
                      ...
                      // Multi-category group (3+ categories)
                      const MULTI_H = 40;
                      const gradStops = cats.map((c, i) => (
                        <stop key={c} offset={`${Math.round((i / (cats.length - 1)) * 100)}%`}
                          stopColor={FINDING_META[c]?.color ?? '#888'} stopOpacity={0.25} />
                      ));
                      return (
                        <g key={`fgzone-multi-${sec.id}`}>
                          <rect x={x} y={y} width={width} height={height} rx={0}
                            fill="rgba(255,255,255,0.015)" stroke="none"
                            style={{ cursor: 'default' }}
                            onClick={(e) => { e.stopPropagation(); toggleGroup(sec.id); }} />
                          {cats.map((c, idx) => (
                            <rect key={c} x={x + idx} y={y + idx} width={width - idx * 2} height={height - idx * 2} rx={0}
                              fill="none" stroke={FINDING_META[c]?.color ?? '#888'} strokeWidth={1.2}
                              strokeOpacity={0.3} strokeDasharray="5 4" style={{ pointerEvents: 'none' }} />
                          ))}
                          <defs>
                            <linearGradient id={`multigrad-${sec.id}`} x1="0%" y1="0%" x2="100%" y2="0%">
                              {gradStops}
                            </linearGradient>
                          </defs>
                          <rect x={x} y={y} width={width} height={MULTI_H} rx={0}
                            fill={`url(#multigrad-${sec.id})`} style={{ pointerEvents: 'none' }} />
                          <text x={x + 10} y={y + 16} fontSize={10} fontWeight={700}
                            fill="rgba(255,255,255,0.9)" style={{ pointerEvents: 'none', userSelect: 'none' }}>
                            {cats.map((c) => `${FINDING_META[c]?.icon ?? ''} ${FINDING_META[c]?.label ?? c}`).join('  ·  ')}
                          </text>
                          <text x={x + width - 50} y={y + 22} fontSize={10} fill="rgba(255,255,255,0.45)"
                            style={{ pointerEvents: 'none', userSelect: 'none' }}>
                            {assetIds.length} node{assetIds.length !== 1 ? 's' : ''}
                          </text>
                          <g style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); toggleGroup(sec.id); }}>
                            <circle cx={x + width - 18} cy={y + MULTI_H / 2} r={10}
                              fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.22)" strokeWidth={1.2} />
                            <text x={x + width - 18} y={y + MULTI_H / 2 + 4} textAnchor="middle"
                              fontSize={10} fill="rgba(255,255,255,0.65)"
                              style={{ pointerEvents: 'none', userSelect: 'none' }}>✕</text>
                          </g>
                        </g>
                  ...
                  {/* ── Collapsed multi-category section cards ── */}
                  {findingSections.filter((s) => !s.isExpanded && s.type === 'multi' && s.assetIds.length > 0).map((sec) => {
                    const { x, y, width: w, height: h } = sec;
                    const cx = x + w / 2; const cy = y + h / 2;
                    const gradStops = sec.cats.map((c, i) => (
                      <stop key={c} offset={`${Math.round((i / Math.max(sec.cats.length - 1, 1)) * 100)}%`}
                        stopColor={FINDING_META[c]?.color ?? '#888'} stopOpacity={0.8} />
                    ));
                    return (
                      <g key={`fgcard-multi-${sec.id}`} style={{ cursor: 'pointer' }}
                        onClick={(e) => { e.stopPropagation(); toggleGroup(sec.id); }}>
                        <defs>
                          <linearGradient id={`multicardgrad-${sec.id}`} x1="0%" y1="0%" x2="100%" y2="0%">
                            {gradStops}
                          </linearGradient>
                        </defs>
                        <rect x={x - 4} y={y - 4} width={w + 8} height={h + 8} rx={12}
                          fill={`url(#multicardgrad-${sec.id})`} opacity={0.06} style={{ pointerEvents: 'none' }} />
                        <rect x={x} y={y} width={w} height={h} rx={9}
                          fill="rgba(14,18,32,0.96)" stroke={`url(#multicardgrad-${sec.id})`} strokeWidth={1.8} strokeOpacity={0.7} />
                        <rect x={x} y={y} width={5} height={h} rx={4}
                          fill={`url(#multicardgrad-${sec.id})`} opacity={0.8} style={{ pointerEvents: 'none' }} />
                        <text x={x + 36} y={cy - 5} textAnchor="start" fontSize={9} fontWeight={700}
                          fill="rgba(255,255,255,0.85)" style={{ pointerEvents: 'none', userSelect: 'none' }}>
                          {sec.cats.map((c) => `${FINDING_META[c]?.icon ?? ''} ${FINDING_META[c]?.label ?? c}`).join(' · ')}
                        </text>
                        <text x={x + 36} y={cy + 9} textAnchor="start" fontSize={8}
...
export default AttackPathGraphV4U5;
