...
const DetectionPreventionExpectationsValidationForm: FunctionComponent<FormProps> = ({ expectation, result, sourceIds = [], onUpdate }) => {
  ...
  const {
    ...
    resolver: zodResolver(zodImplement<{
      ...
      expectation_score: z.number().min(0, { message: t('Should be superior or equal to 0') }).max(100, { message: t('Should be inferior or equal to 100') }).int(t('Score must be a whole number')),
