...
const PlatformGroupPopover: FunctionComponent<Props> = ({
  ...
  const handleOpenUsers = useCallback(() => { setIsUsersOpen(true); }, []);
  const handleCloseUsers = useCallback(() => { setIsUsersOpen(false); }, []);

  const [isRolesOpen, setIsRolesOpen] = useState(false);
  const handleOpenRoles = useCallback(() => { setIsRolesOpen(true); }, []);
  const handleCloseRoles = useCallback(() => { setIsRolesOpen(false); }, []);

  const submitUpdateUsers = useCallback((userIds: string[]) => {
    updatePlatformGroupUsers(platformGroup.platform_group_id, { platform_group_users: userIds });
  }, [platformGroup.platform_group_id]);

  const submitUpdateRoles = useCallback((roleIds: string[]) => {
    updatePlatformGroupRoles(platformGroup.platform_group_id, { platform_group_platform_roles: roleIds });
  }, [platformGroup.platform_group_id]);
  ...
  const entries = useMemo(() => {
    ...
    if (actions.includes('Update')) {
      result.push({ label: t('Update'), action: handleOpenEdit, userRight: canManage });
    }
    if (actions.includes('Manage users')) {
      result.push({ label: t('Manage users'), action: handleOpenUsers, userRight: canManage });
    }
    if (actions.includes('Manage roles')) {
      result.push({ label: t('Manage roles'), action: handleOpenRoles, userRight: canManage });
    }
    if (actions.includes('Delete')) {
      result.push({ label: t('Delete'), action: handleOpenDelete, userRight: canDelete });
    }
    return result;
