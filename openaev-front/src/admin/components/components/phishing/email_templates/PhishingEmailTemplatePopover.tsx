...
const PhishingEmailTemplatePopover: FunctionComponent<Props> = ({ emailTemplate, inList = false }) => {
  ...
  const onSubmitEdit = async (data: PhishingEmailTemplateFormInput) => {
    await dispatch(updatePhishingEmailTemplate(emailTemplate.phishing_email_template_id, data));
    setOpenEdit(false);
  };
