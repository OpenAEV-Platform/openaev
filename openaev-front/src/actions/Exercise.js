...
import { arrayOfExercises, arrayOfInjectexpectations, arrayOfTeams, exercise, injectexpectation } from './schemas';

export const fetchExercises = () => dispatch => getReferential(arrayOfExercises, '/api/exercises')(dispatch);

export const fetchExercisesById = exerciseIds => dispatch => postReferential(arrayOfExercises, '/api/exercises/search-by-id', exerciseIds, undefined, false)(dispatch);

export const searchExercises = paginationInput => simplePostCall('/api/exercises/search', paginationInput);

export const fetchExercise = exerciseId => dispatch => getReferential(exercise, `/api/exercises/${exerciseId}`)(dispatch);

export const fetchExerciseInjectExpectations = exerciseId => dispatch => getReferential(
  arrayOfInjectexpectations,
  `/api/exercises/${exerciseId}/expectations`,
)(dispatch);

export const addExercise = data => dispatch => postReferential(exercise, '/api/exercises', data)(dispatch);

export const duplicateExercise = exerciseId => dispatch => postReferential(exercise, `/api/exercises/${exerciseId}`, null)(dispatch);
...
export const fetchExerciseTeams = exerciseId => (dispatch) => {
  ...
  return getReferential(arrayOfTeams, uri)(dispatch);
};
...
export const fetchPlayerExercise = (exerciseId, userId) => (dispatch) => {
  ...
  return getReferential(exercise, uri)(dispatch);
};
