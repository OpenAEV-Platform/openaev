...
import { type Widget, type WidgetInput, type WidgetLayout } from '../../utils/api-types';
...
export const createCustomDashboardWidget = (customDashboardId: string, input: WidgetInput) => {
  return simplePostCall<Widget>(`${CUSTOM_DASHBOARD_URI}/${customDashboardId}/widgets`, input);
};

export const updateCustomDashboardWidget = (customDashboardId: string, widgetId: string, input: WidgetInput) => {
  return simplePutCall<Widget>(`${CUSTOM_DASHBOARD_URI}/${customDashboardId}/widgets/${widgetId}`, input);
};

export const updateCustomDashboardWidgetLayout = (customDashboardId: string, widgetId: string, input: WidgetLayout) => {
  return simplePutCall<Widget>(`${CUSTOM_DASHBOARD_URI}/${customDashboardId}/widgets/${widgetId}/layout`, input, {}, true, false);
};
