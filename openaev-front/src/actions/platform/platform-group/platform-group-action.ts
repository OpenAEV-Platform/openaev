...
import {arrayOfPlatformGroups, PLATFORM_GROUP_SCHEMA_KEY, platformGroup} from './platform-group-schema';
...
export const fetchPlatformGroup = (platformGroupId: PlatformGroupOutput['platform_group_id']) => (dispatch: Dispatch) => {
  const uri = `${PLATFORM_GROUPS_URI}/${platformGroupId}`;
  return getReferential(arrayOfPlatformGroups, uri)(dispatch);
};
