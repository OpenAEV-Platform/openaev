...
// plain /api/attack-path/... paths and we assert exactly those.
...
describe('attack path POC actions', () => {
  ...
  it('fetchAttackPathGraph hits the graph endpoint without a mode by default', async () => {
    ...
    expect(simpleCall).toHaveBeenCalledWith('/api/attack-path/simulations/SIM-1/graph');
  });

  it('fetchAttackPathGraph forwards the mode as a query parameter', async () => {
    const { fetchAttackPathGraph } = await importActions();
    await fetchAttackPathGraph('SIM-1', 'collapsed');
    expect(simpleCall).toHaveBeenCalledWith('/api/attack-path/simulations/SIM-1/graph?mode=collapsed');
  });
  ...
  it('fetchEndpointFindings encodes the endpoint ref', async () => {
    ...
    expect(simpleCall).toHaveBeenCalledWith('/api/attack-path/simulations/SIM-1/endpoint/findings?ref=asset%2F01');
  });
  ...
  it('fetchEndpointRelations encodes the endpoint ref', async () => {
    ...
    expect(simpleCall).toHaveBeenCalledWith('/api/attack-path/simulations/SIM-1/endpoint/relations?ref=10.0.0.1');
  });
