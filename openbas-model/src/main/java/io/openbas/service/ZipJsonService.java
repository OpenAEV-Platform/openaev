...
public class ZipJsonService<T extends Base> {
    ...
    public byte[] handleExportResource(T entity, Map<String, byte[]> extras, JsonApiDocument<ResourceObject> resource) throws IOException {
        if (extras == null) {
            extras = new HashMap<>();
        }
    ...
    public JsonApiDocument<ResourceObject> handleImport(
            byte[] fileBytes, String nameAttributeKey, IncludeOptions includeOptions)
            throws IOException {
