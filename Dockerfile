...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -q && DEBIAN_FRONTEND=noninteractive apt-get install -qq -y tini && DEBIAN_FRONTEND=noninteractive apt-get install -qq -y --only-upgrade libsystemd0 && rm -rf /var/lib/apt/lists/*
