 ...
 * value for these rows. Secrets are never stored - {@code ai_target_api_key_variable} only names the
 * injector configuration key that resolves the credential at execution time.
 */
@Component
public class V5_26__Add_ai_target_columns extends BaseJavaMigration {
    ...
    try (Statement statement = context.getConnection().createStatement()) {
      statement.execute(
          "ALTER TABLE assets ADD COLUMN IF NOT EXISTS ai_target_provider varchar(255);"
              + "ALTER TABLE assets ADD COLUMN IF NOT EXISTS ai_target_endpoint text;"
