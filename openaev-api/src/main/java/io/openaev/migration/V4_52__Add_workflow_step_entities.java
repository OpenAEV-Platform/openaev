...
public class V4_52__Add_workflow_step_entities extends BaseJavaMigration {
  ...
  public void migrate(Context context) throws Exception {
    try (Statement select = context.getConnection().createStatement()) {
      ...
      select.execute(
          """
      BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE tyname = 'step_type' ) then
              CREATE TYPE step_type AS ENUM ('INJECT_EXECUTION', 'TIMESTAMP', 'CONDITION');
        END IF;
       END;
      
      BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE tyname = 'step_status' ) then
      ...
      select.execute(
        ...
        CREATE TABLE edges (
            ...
            step_children_id VARCHAR(255) NOT NULL UNIQUE REFERENCES steps(step_id) ON DELETE CASCADE,
      ...
      select.execute(
        ...
        CREATE TYPE step_status AS ENUM ('AND', 'OR', 'EQ', 'NEQ', 'IS_NULL', 'IS_NOT_NULL', 'GT', 'GTE', 'LT', 'LTE', 'IN', 'NIN','AFTER','BEFORE');
        CREATE TABLE conditions (
            ...
            condition_edge_template_id VARCHAR(255) NOT NULL REFERENCES edges(edge_id) ON DELETE CASCADE,
            condition_edge_execution_id VARCHAR(255) NOT NULL REFERENCES edges(edge_id) ON DELETE CASCADE,
            condition_created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
            condition_updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
            UNIQUE (condition_edge_execution_id, condition_key, condition_value, condition_type)
          );
        """);
    }
  }
