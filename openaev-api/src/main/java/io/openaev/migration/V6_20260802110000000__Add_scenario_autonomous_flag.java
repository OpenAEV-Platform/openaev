 ...
 *
 * <p>An autonomous (AI-driven) run owns exactly one scenario, but the link lived only on the
 * {@code autonomous_runs} row (keyed by {@code autonomous_run_scenario_id}), so the frontend had to
 * probe the autonomous-run lookup endpoint on EVERY scenario load just to discover a plain manual
 ...
 *
 * <p>Backfills existing autonomous scenarios (those already referenced by an {@code autonomous_runs}
 * row) to {@code true} so their AI cockpit keeps rendering after the upgrade.
 *
 ...
 */
@Component
public class V6_20260802110000000__Add_scenario_autonomous_flag extends BaseJavaMigration {
