...
/**
 * Repairs {@code payloads.payload_arguments} rows that were persisted as the Hypersistence {@code
 * JsonType} wrapper object ({@code {"type":"json","value":"[...]","null":false}}) instead of a
 * plain JSON array.
 *
 ...
 * touched array-shaped {@code payload_arguments} (to strip {@code subtype}) and the {@code
 * status_payload_output} column; its {@code json_typeof(...) = 'array'} guard skipped the
 * object-wrapped shape, so those rows survived. This migration closes that gap.
 *
 * <p>Idempotent and re-runnable: only object-shaped rows carrying a {@code value} key are
 * rewritten; once unwrapped they are arrays and no longer match the guard, so a second run (or a
 * fresh install that never had the bad shape) is a no-op. Lock-light single-column DML on a small
 * table.
 */
@Component
