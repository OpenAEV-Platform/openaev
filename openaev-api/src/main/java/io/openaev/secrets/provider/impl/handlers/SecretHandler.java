...
public interface SecretHandler {
   ...
   *
   * <p>Opt-in by design: the default answers {@link SecretValidationResult#unsupported()}, so a
   ...
   * failure surface as {@code INACTIVE} — see {@link SecretValidationResult#inactive(String)}.
   *
   * @param secret the secret to check
   * @return the validation outcome, never null
   */
  default SecretValidationResult validateConnection(Secret secret) {
    return SecretValidationResult.unsupported();
  }
}
