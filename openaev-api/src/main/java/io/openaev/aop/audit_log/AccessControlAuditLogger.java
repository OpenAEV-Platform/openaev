...
public class AccessControlAuditLogger {
    ...
    public CompletableFuture<Boolean> auditEvent(
        ...
        if (
            !isHttpDelete && entitySnapshot != null && entityId != null && entityType != null &&
            ("update".equals(eventScope) || "create".equals(eventScope) || "duplicate".equals(eventScope))
        ...
        else if ("update".equals(eventScope) && result instanceof Collection<?> collection && !collection.isEmpty()) {
            List<Base> childEntities = collection.stream().filter(Base.class::isInstance).map(Base.class::cast).toList();

            if (!childEntities.isEmpty()) {
                Class<?> expectedParentClass = ResourceManagerUtils.getClassByResource(resourceType);

                // Verify these are truly child entities, not the parent type itself
                if (expectedParentClass != null && !expectedParentClass.isInstance(childEntities.getFirst())) {
                    ResourceType childType = ResourceManagerUtils.getResourceByClass(childEntities.getFirst().getClass());
                    ...
                    for (int i = 0; i < childEntities.size(); i++) {
                        ...
                        CompletableFuture<Boolean> subFuture = logMutationEvent(
                                "create",
                                eventStatus,
                    ...
                    CompletableFuture<Boolean> future = all.thenApply(v ->
                            futures.stream()
        ...
        CompletableFuture<Boolean> future = logMutationEventIfDifferentSnapshots(
                eventScope,
                eventStatus,
                resourceType,
                resourceId,
    ...
    public CompletableFuture<Boolean> logMutationEventIfDifferentSnapshots(
        ...
        try {
            status = logService.logMutationEventIfDifferentSnapshots(
                    eventScope,
                    eventStatus,
                    resourceType,
                    entityId,
    ...
    public CompletableFuture<Boolean> logAuthEvent(
        ...
        try {
            status = logService.logAuthEvent(
                    eventScope,
                    eventStatus,
                    provider,
                    reason,
    ...
    public CompletableFuture<Boolean> logMutationEvent(
        ...
        try {
            status = logService.logMutationEvent(
                    eventScope,
                    eventStatus,
                    resourceType,
                    entityId,
