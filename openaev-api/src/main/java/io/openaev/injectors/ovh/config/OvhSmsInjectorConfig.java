...
public class OvhSmsInjectorConfig extends BaseIntegrationConfiguration {
    ...
    @IntegrationConfigKey(key = "OVH_SMS_AS", description = "OVHCloud Access Secret", isRequired = true,
            valueFormat = CatalogConnectorConfiguration.CONNECTOR_CONFIGURATION_FORMAT.PASSWORD)
    @NotBlank
    private String as;

    @IntegrationConfigKey(key = "OVH_SMS_CK", description = "OVHCloud consumer key", isRequired = true)
    @NotBlank
    private String ck;

    @IntegrationConfigKey(key = "OVH_SMS_SERVICE", description = "OVHCloud Service Identifier", isRequired = true)
    @NotBlank
    private String service;

    @IntegrationConfigKey(key = "OVH_SMS_SENDER", description = "OVHCloud sms sender", isRequired = true)
    @NotBlank
