 ...
 * HOUR digests fire on the hour and ignore {@code triggerTime}. MONTH days exceeding the current
 * month's length are clamped to its last day, so a digest configured for the 31st still fires in
 * shorter months.
 */
public final class NotificationTriggerTimeUtils {
  ...
  public static boolean isTimeTrigger(
    ...
    return switch (period) {
      ...
      case MONTH -> isMonthTrigger(triggerTime, utcNow, hourMinute);
    };
  }

  private static boolean isMonthTrigger(
      String triggerTime, ZonedDateTime utcNow, String hourMinute) {
    if (triggerTime == null) {
      return false;
    }
  ...
  /**
   * Returns the start of the aggregation window ending at {@code to}. Calendar-aware for MONTH (one
   * calendar month back) so the window matches the calendar-based firing cadence instead of a fixed
   * 31 days.
   */
  public static Instant windowStart(NotificationTriggerPeriod period, Instant to) {
    return switch (period) {
