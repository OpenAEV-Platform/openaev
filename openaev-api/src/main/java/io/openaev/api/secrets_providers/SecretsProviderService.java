...
@Service
public class SecretsProviderService extends AbstractConnectorService<SecretsProvider, SecretsProviderOutput> {
    ...
    protected SecretsProvider getConnectorById(String id) {
        return secretService.getAllProviders().stream().filter(sp -> id.equals(sp.getId())).findFirst().orElse(null);
    }

    @Override
    protected SecretsProviderOutput mapToOutput(SecretsProvider connector, CatalogConnector catalogConnector, ConnectorInstance instance, boolean existingConnector) {
