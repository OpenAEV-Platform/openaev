 ...
 * both the workflow rule {@code source} (ASSET / ASSET_GROUP / TEAM / MANUAL / CSV) and the
 * orchestrator target-kind {@code type} (ASSETS / ASSETS_GROUPS / TEAMS / MANUAL), plus the value
 * ({@code id} - an entity id, or a raw IP / CIDR / hostname for manual rules) and a resolved
...
public class AutonomousScopeEntry {
  ...
  @JsonProperty("source")
  @Schema(description = "Workflow scope-rule source: ASSET, ASSET_GROUP, TEAM, MANUAL, CSV")
  private String source;

  @JsonProperty("type")
  @Schema(
      description =
          "Orchestrator target kind: ASSETS, ASSETS_GROUPS, TEAMS (or MANUAL for a raw"
