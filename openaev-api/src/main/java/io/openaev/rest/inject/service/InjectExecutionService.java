...
public class InjectExecutionService {
  ...
  @VisibleForTesting
  public Map<String, Long> processInjectExecution(
      Inject inject,
    ...
    results.put("structured", Instant.now());
    results.put("computeStructured", Instant.now());
    results.put("checkCveExpectation", Instant.now());
    results.put("updateInjectStatus", Instant.now());
    results.put("addEndDate", Instant.now());
    results.put("extractFindings", Instant.now());
        ...
        .collect(
            Collectors.toMap(
                ...
                e -> {
                  switch (e.getKey()) {
                    case "structured":
                      return e.getValue().toEpochMilli() - start.toEpochMilli();
                    case "computeStructured":
                      return e.getValue().toEpochMilli() - results.get("structured").toEpochMilli();
                    case "checkCveExpectation":
                      return e.getValue().toEpochMilli()
                          - results.get("computeStructured").toEpochMilli();
                    case "updateInjectStatus":
                      return e.getValue().toEpochMilli()
                          - results.get("checkCveExpectation").toEpochMilli();
                    case "addEndDate":
                      return e.getValue().toEpochMilli()
                          - results.get("updateInjectStatus").toEpochMilli();
                    case "extractFindings":
                      return e.getValue().toEpochMilli() - results.get("addEndDate").toEpochMilli();
                  }
