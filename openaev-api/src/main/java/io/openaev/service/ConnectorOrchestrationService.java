...
public class ConnectorOrchestrationService {
  ...
  /**
   * Updates the requested status of a connector instance.
   * Validates license and XTM Composer connectivity if required.
   *
   ...
   */
  public ConnectorInstance updateRequestedStatus(String connectorInstanceId, ConnectorInstance.REQUESTED_STATUS_TYPE requestedStatus){
    validateEnterpriseLicense();

    ConnectorInstance instance = connectorInstanceService.connectorInstanceById(connectorInstanceId);
  ...
  private void validateNoDuplicateInstance(String catalogId){
    List<ConnectorInstance> existingInstances = connectorInstanceService.findAllByCatalogConnectorId(catalogId);
    if (!existingInstances.isEmpty()) {
      throw new IllegalArgumentException("ConnectorInstance with CatalogConnector id " + catalogId + " already exists");
    }
  }

  private void validateNoDuplicateConnector(String catalogConnectorSlug, CatalogConnector.CONNECTOR_TYPE catalogConnectorType){
    BaseConnectorEntity connector;
    if(CatalogConnector.CONNECTOR_TYPE.COLLECTOR.equals(catalogConnectorType)){
      connector = collectorService.findCollectorByType("openaev_"+catalogConnectorSlug).orElse(null);
    ...
    if(connector != null) {
      throw new IllegalArgumentException("Connector with slug " + catalogConnectorSlug + " already exists");
    }
  }

  private void validateCatalogInstanceCreation(String catalogConnectorId, String catalogConnectorSlug, CatalogConnector.CONNECTOR_TYPE catalogConnectorType) {
