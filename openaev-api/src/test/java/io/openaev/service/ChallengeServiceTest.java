...
public class ChallengeServiceTest extends IntegrationTest {
    ...
    void shouldRunPlayerChallenges() {
        ...
        InjectExpectation expectation = InjectExpectationFixture.createExpectationWithTypeAndStatus(
        ...
        when(injectExpectationRepository.findChallengeExpectationsByExerciseAndUser("test", "test")).thenReturn(expectations);
    ...
    void shouldValidateChallenges() {
        ...
        // MOCK
        when(exerciseRepository.findById("test")).thenReturn(Optional.of(exercise));
        when(injectExpectationRepository.findChallengeExpectationsByExerciseAndUser("test", "test")).thenReturn(playerExpectations);
        when(challengeRepository.findById("test")).thenReturn(Optional.of(challenge));
        when(injectExpectationRepository.findByUserAndExerciseAndChallenge(user.getId(), "test", "test")).thenReturn(playerExpectations);
        when(challengeAttemptService.getChallengeAttempt(any())).thenReturn(Optional.empty());
        when(injectExpectationService.updateInjectExpectation(any(), (ExpectationUpdateInput) any())).thenReturn(new InjectExpectation());

        // EXECUTE
        SimulationChallengesReader reader = challengeService.validateChallenge("test","test", input, user);
        ...
        assertNotNull(reader.getExerciseChallenges().getFirst());
        assertNotNull(reader.getExerciseChallenges().getFirst().getChallenge());
        assertEquals(challenge.getName(), reader.getExerciseChallenges().getFirst().getChallenge().getName());
    }
