...
class UsernameOutputProcessorTest {
  ...
  private final UsernameOutputProcessor processor = new UsernameOutputProcessor();
  ...
  void shouldReturnTrueWhenUsernamePresent() throws Exception {
    JsonNode node = objectMapper.readTree("{\"username\": \"alice\", \"domain\": \"CORP\", \"host\": \"dc01\"}");
  ...
  void shouldReturnMultipleAssetIdsWhenAssetIdIsArray() throws Exception {
    JsonNode node = objectMapper.readTree("{\"username\": \"alice\", \"asset_id\": [\"a1\", \"a2\"]}");
    List<String> result = processor.toFindingAssets(node);
