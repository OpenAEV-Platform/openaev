...
@DisplayName("ScenarioLessonsApi scopes the template lookup to the caller's tenants")
class ScenarioLessonsApiTenantIsolationTest extends IntegrationTest {

  private static final String APPLY_TEMPLATE =
      "/api/tenants/{tenantId}/scenarios/{scenarioId}/lessons_apply_template/{lessonsTemplateId}";
  ...
  void seedTwoTenantsWithOneTemplateEach() throws Exception {
    tenantA = tenantHelper.createTenantWithCurrentUser("scenario-lessons-a").getId();
    String tenantB = tenantHelper.createTenantWithCurrentUser("scenario-lessons-b").getId();
    templateA = seedTemplate(tenantA, "scenario-template-a");
    templateB = seedTemplate(tenantB, "scenario-template-b");
