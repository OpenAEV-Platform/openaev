...
/**
 * Guards that the production configuration keeps {@code executors} activated in v2. The table is
 ...
 */
@DisplayName("Production config keeps executors activated (v2)")
class ExecutorActivationConfigTest {

  @Test
  @DisplayName("openaev.tenant.active-tables in application.properties contains executors")
  void prodConfigActivatesExecutors() throws Exception {
    ...
    assertTrue(
        active.contains("executors"),
        "executors must stay in openaev.tenant.active-tables: its v1 @Filter was removed, so"
