...
class TemplateHelperTest {
  ...
  @Nested
  @DisplayName("Restricted built-in classes")
  class SaferResolver {
    ...
    void given_restricted_builtin_classes_should_all_produce_fallback_output()
      ...
      for (String restricted : RESTRICTED_CLASSES) {
        ...
        assertTrue(
            rendered.contains("${"),
            shortName
                + ": expression must be preserved as literal text — ALLOWS_NOTHING_RESOLVER may be missing");
      }
    }

    @Test
    @DisplayName(
        "given mixed content with restricted class should resolve variables and preserve restricted expression")
    void given_mixed_content_should_resolve_variables_and_preserve_restricted_expression()
      ...
      assertTrue(
          rendered.contains("${"),
          "Restricted expression must be preserved as literal text — ALLOWS_NOTHING_RESOLVER may be missing");
    }
  }
