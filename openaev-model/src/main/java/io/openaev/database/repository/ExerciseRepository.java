  ...
  @Query(
      value =
          "WITH changed_exercises AS ("
              ...
              + "COALESCE(GREATEST(ex.exercise_updated_at, max(inj.inject_updated_at), max(ic.injector_contract_updated_at)), ex.exercise_updated_at) as exercise_injects_updated_at, "
              + "array_agg(DISTINCT et.tag_id) FILTER ( WHERE et.tag_id IS NOT NULL ) as exercise_tags, "
