...
public class OcsfObjectFile extends OcsfObject {
   ...
   * Windows OS, most PE files contain a <a
   * href="https://learn.microsoft.com/en-us/windows/win32/menurc/versioninfo-resource">VERSIONINFO</a>
   * resource from which the internal name can be obtained. On macOS, binaries can optionally embed
   * a copy of the application's Info.plist file which in turn contains the name of the executable.
   */
   ...
   * folder extended attribute.For example: Windows alternate data stream attributes (ADS stream
   * name, ADS size, etc.), user-defined or application-defined attributes, ACL, owner, primary
   * group, etc. Examples from DCS:
   *
   * <ul>
