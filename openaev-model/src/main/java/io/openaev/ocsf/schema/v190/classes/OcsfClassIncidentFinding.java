...
@lombok.Getter
public class OcsfClassIncidentFinding extends OcsfClass {
  ...
  /**
   * The normalized identifier of the incident activity. Use <code>1</code> (Create) when an
   * incident is first opened, <code>2</code> (Update) when an existing incident is modified (e.g.,
   * new findings added, priority changed, assignee updated), and <code>3</code> (Close) when the
  ...
  /**
   * The incident activity name, as defined by the <code>activity_id</code>. When <code>activity_id
  ...
  /**
   * The normalized identifier of the confidence level for this incident, representing a summary
   * across all aggregated findings. It reflects the accuracy of the analytics that produced those
   * findings. A lower confidence value indicates a wider finding scope that may produce noisy or
   * potentially non-malicious reports. When the value is <code>99</code> (Other), the <code>
   * confidence</code> attribute must contain the incident-specific label.
   */
  ...
  /**
   * The confidence score assigned to the incident by the system or analyst, expressed on a
   * source-specific numeric scale. This score provides finer-grained confidence information than
   * the categorical <code>confidence_id</code> value. When <code>confidence_id</code> is <code>99
   * </code> (Other), the <code>confidence</code> attribute should reflect the same assessment if
   * both are populated.
   */
  ...
  /** A list of <code>finding_info</code> objects. */
  @com.fasterxml.jackson.annotation.JsonProperty(value = "finding_info_list")
  private java.util.List<io.openaev.ocsf.schema.v190.objects.OcsfObjectFindingInfo>
      findingInfoListField;
   ...
   * explanations, or observations by analysts related to the incident over time.
   */
  ...
  /**
   * The incident lifecycle status label, normalized to the caption of the <code>status_id</code>
  ...
  /**
   * The normalized incident lifecycle status identifier. This tracks the incident management
   * workflow from initial triage through resolution. Producers should set this to reflect the
   * current state in their incident management system.
   */
  ...
  /**
   * The incident creation time - when the incident was first opened, not when the underlying
