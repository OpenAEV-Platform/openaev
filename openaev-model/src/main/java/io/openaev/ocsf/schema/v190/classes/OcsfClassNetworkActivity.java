...
@lombok.Getter
public class OcsfClassNetworkActivity extends OcsfClass {
  ...
  /**
   * The destination endpoint of the network activity — typically the responder. In bi-flow or
   * asymmetric flow scenarios, this endpoint may represent the true initiator of the original
   * communication. Use <code>initiator_id</code> to determine which endpoint actually initiated the
   * communication.
   */
  ...
  /**
   * The endpoint that initiated the network communication, normalized to the caption of <code>
   * initiator_id</code>. In the case of <code>Other</code>, it is defined by the event source.
   */
  @com.fasterxml.jackson.annotation.JsonProperty(value = "initiator")
  private io.openaev.ocsf.schema.v190.datatypes.OcsfDatatypeStringT initiatorField;

  /**
   * The normalized identifier of the endpoint that initiated the network communication. Use this
   * field to disambiguate source and destination roles in bi-directional or asymmetric flow
   * scenarios.
   */
  @com.fasterxml.jackson.annotation.JsonProperty(value = "initiator_id")
  private io.openaev.ocsf.schema.v190.datatypes.OcsfDatatypeIntegerT initiatorIdField;
  ...
  /**
   * <code>true</code> denotes that <code>src_endpoint</code> and <code>dst_endpoint</code>
   * correctly identify the initiator and responder respectively. <code>false</code> denotes that
   * the event source has arbitrarily assigned one peer to <code>src_endpoint</code> and the other
   * to <code>dst_endpoint</code>, in other words that initiator and responder are not being
   * asserted. This can occur, for example, when the event source is a network appliance that has
   * not observed the initiation of a given connection. In the absence of this attribute,
   * interpretation of the initiator and responder is implementation-specific.
   */
  @com.fasterxml.jackson.annotation.JsonProperty(value = "is_src_dst_assignment_known")
  private io.openaev.ocsf.schema.v190.datatypes.OcsfDatatypeBooleanT isSrcDstAssignmentKnownField;
  ...
  /**
   * The source endpoint of the network activity — typically the initiator. In bi-flow or asymmetric
   * flow scenarios, this endpoint may represent the response side of communication originally
   * initiated by the destination. Use <code>initiator_id</code> to determine which endpoint
   * actually initiated the communication.
   */
