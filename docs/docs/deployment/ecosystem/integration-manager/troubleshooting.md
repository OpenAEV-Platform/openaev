...
If XTM Composer cannot connect to OpenAEV:

### 1. Verify URL and token

Test the connection directly using curl:
```bash
curl -H "Authorization: Bearer YOUR_TOKEN" https://openaev.example.com/api/settings/version
```
...
```bash
# Test DNS resolution
ping openaev.example.com

# Test port connectivity
nc -zv openaev.example.com 443
```
...
**"Successfully connected to OpenAEV"**: Connection established successfully
