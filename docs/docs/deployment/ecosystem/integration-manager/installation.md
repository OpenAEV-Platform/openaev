...
Note: The Kubernetes installation method described here assumes that OpenAEV is already deployed on a Kubernetes cluster.
...
The [OpenAEV Docker repository](https://github.com/OpenAEV-Platform/docker) provides a complete `docker-compose.yml` that already includes:   
- XTM Composer service pre-configured   
- Automatic RSA key generation service   
- Full OpenAEV stack integration   
