...
| openaev.healthcheck.storage-usage-cache-duration | OPENAEV_HEALTHCHECK_STORAGE-USAGE-CACHE-DURATION | PT4H      | Validity of the storage usage metrics returned by `/api/health?details=true` (ISO-8601 duration). They are cached because computing them walks the object storage listing and queries the engine |
