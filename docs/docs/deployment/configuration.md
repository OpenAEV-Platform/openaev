...
| openaev.unsecured-certificate   | OPENAEV_UNSECURED_CERTIFICATE   | `false`                 | Turn on to authorize self-signed or unsecure ssl certificate                                                                                                                                                                                                                                                                                                                                                                |
| openaev.with-proxy              | OPENAEV_WITH_PROXY              | `false`                 | Turn on to authorize environment with proxy                                                                                                                                                                                                                                                                                                                                                                                |
...
| openaev.agent.queue-threshold              | OPENAEV_AGENT_QUEUE_THRESHOLD               | 0            | Max number of pending jobs per agent before marking it overloaded. 0 disables the check. |
##### OpenAEV Agent
s
...
| Parameter                   | Environment variable        | Default value        | Description                                                                                                                                                                                          |
|:-------------------------------|:--------------------------------|:-------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| openaev.default-mailer      | OPENAEV_DEFAULT_MAILER      | no-reply@openaev.io  | Default sender email address, used to pre-fill new Scenarios/Simulations. This field is read-only in the UI (locked to prevent spoofing — see [#4726](https://github.com/OpenAEV-Platform/openaev/issues/4726)); only the sender name below can be edited from the UI |
| openaev.default-mailer-name | OPENAEV_DEFAULT_MAILER_NAME | no-reply             | Default sender display name ("Sender email from"), pre-fills the sender name on new Scenario/Simulation creation. Editable per-Scenario/Simulation afterwards in the UI                              |
| openaev.default-reply-to    | OPENAEV_DEFAULT_REPLY_TO    | contact@openaev.io   | Default Reply-To address used on outgoing emails                                                                                                                                                     |
