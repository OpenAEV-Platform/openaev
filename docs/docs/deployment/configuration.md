...
| openaev.healthcheck.storage-usage-cache-duration | OPENAEV_HEALTHCHECK_STORAGE_USAGE_CACHE_DURATION | PT4H      | Max age of the storage usage metrics returned by `/api/health?details=true` (ISO-8601 duration, e.g. PT30M). No effect on the health check itself. Once expired, the next call with details=true recomputes them synchronously, walking the whole object storage listing, so low values make some calls slow rather than refreshing anything in the background |
