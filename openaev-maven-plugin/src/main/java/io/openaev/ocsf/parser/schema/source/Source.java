...
public class Source {
  ...
  public Source(Version version, SchemaDimension dimension, PluginContext ctx) throws IOException {
    this(version, dimension, ctx, null);
  }

  public Source(Version version, SchemaDimension dimension, PluginContext ctx, String name)
      throws IOException {
    ...
    initialiseFileResource(dimension, ctx, name);
  }

  protected void initialiseFileResource(SchemaDimension dimension, PluginContext ctx, String name)
      throws IOException {
    switch (dimension) {
      ...
      case SINGLE_OBJECT -> this.fileResource = new SingleObjectResource(version, ctx, name);
      case SINGLE_CLASS -> this.fileResource = new SingleClassResource(version, ctx, name);
    }
  }
  ...
  public void refresh() throws IOException {
    this.fileResource.write(client.fetch(dimensionToEndpointMap.get(this.dimension), this.name));
  }
