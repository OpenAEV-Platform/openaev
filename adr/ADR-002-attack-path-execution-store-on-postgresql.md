# ADR-002: Attack path execution store on PostgreSQL
...
4. Operational simplicity. Prefer not to add a second datastore to install, upgrade, and keep in sync.
...
- Rebuilding the whole graph of a very large single simulation is expensive (seconds and gigabytes of heap for the outliers), and per-simulation cost is not volume-independent once the wide-row table outgrows cache. This is the front render ceiling, and it is why the front renders the collapsed graph by default. Bringing a large simulation under the latency target would need a collapsed or aggregated backend response (counts and endpoint groups, not every row) or pagination.
