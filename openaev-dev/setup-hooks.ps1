...
#
# Usage:
#   cd openaev-dev; .\setup-hooks.ps1              # from openaev-dev/
#   powershell -File openaev-dev\setup-hooks.ps1    # from project root
#
# Prerequisites:
#   - Git for Windows (provides bash for running the hooks)
...
if (-not (Test-Path (Join-Path $RootDir ".git") -PathType Container)) {
...
[System.IO.File]::WriteAllText($PrepareCommitMsgPath, $PrepareCommitMsgHook.Replace("`r`n", "`n"))
