...
const STAGES_REACHING_INSTALL = ['builder-front'];
 ...
 */
const withoutComments = (body: string): string => body
  .split('\n')
...
for (const yamlFile of yamlFiles) {
  ...
  for (const body of splitSteps(content)) {
    ...
    if (composite) compositeCalls.push({ source, action: composite, body });
...
describe('CI wiring for the private design-system dependency', () => {
  ...
  it.each(buildSteps)(
    ...
    async ({ files, body, target }) => {
      const reachesInstall = target === undefined
        || STAGES_REACHING_INSTALL.some((stage) => target.startsWith(stage));
      const needs = reachesInstall
        && (await Promise.all(files.map(dockerfileNeedsSecret))).some(Boolean);
      expect(withoutComments(body).includes(`${SECRET_ID}=`)).toBe(needs);
    },
  );

  it.each(compositeCalls.filter(({ action }) => compositeInputs.has(action)))(
  ...
  it.each(containerRuns)('$source / $name runs its container with the credential it needs', async ({ body }) => {
    expect(new RegExp(`-e\\s+${SECRET_NAME}\\b`).test(withoutComments(body)))
      .toBe(await containerRunNeedsSecret(body));
  });
});
