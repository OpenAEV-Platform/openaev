...
public class InitStarterPackCommandLineRunner implements CommandLineRunner {
    ...
    private void importScenariosFromResources() {
        listFilesInResourceFolder(Config.SCENARIOS_FOLDER_NAME)
                .forEach(resourceToAdd -> {
                    try {
                        this.importService.handleInputStreamFileImport(resourceToAdd.getInputStream(), null, null);
                        log.info("Successfully imported StarterPack scenario file : {}", resourceToAdd.getFilename());
                    } catch (Exception e) {
                        log.error("Failed to import StarterPack scenario file : {}; cause {}", resourceToAdd.getFilename(), e.getMessage());
                    }
                });
    }
    ...
    private void importDashboardsFromResources() {
        listFilesInResourceFolder(Config.DASHBOARDS_FOLDER_NAME)
                .forEach(resourceToAdd -> {
                    try {
                        this.zipJsonService.handleImport(resourceToAdd.getContentAsByteArray(), "custom_dashboard_name", null);
                        log.info("Successfully imported StarterPack dashboard file : {}", resourceToAdd.getFilename());
                    } catch (Exception e) {
                        log.error("Failed to import StarterPack dashboard file : {}; cause {}", resourceToAdd.getFilename(), e.getMessage());
                    }
                });
    }
    ...
    private List<Resource> listFilesInResourceFolder(String folderName) {
        try {
            ...
            return Arrays.stream(
                resolver.getResources("classpath:" +  Config.STARTERPACK_KEY + "/" + folderName + "/*"))
                .toList();
        } catch (Exception e) {
            log.error("Failed to import StarterPack files from resource folder {}; cause {}", Config.STARTERPACK_KEY + "/" + folderName, e.getMessage());
